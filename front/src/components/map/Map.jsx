...
const MapPrevi = ({
  ...
  return (
    <article>
      <MapContainer
        center={[ville.coord.lat, ville.coord.lon]}
        zoom={zoom}
      ...
      {location.pathname === "/" && (
        <div className="data-ville">
          <h3>
            {" "}
            <Link
              to={`/prevision/${ville.name.toLowerCase()}`}
              state={{ lat: ville.coord.lat, lon: ville.coord.lon }}
            >
              {ville.name}
            </Link>{" "}
          </h3>
      ...
      {location.pathname !== "/" && (
        <div className="data-ville">
          ...
          <h3>
            <span>{formatDay(ville.dt, ville.timezone)}</span>{" "}
            <span>{formatDateComplete(ville.dt, ville.timezone)}</span>
          </h3>
